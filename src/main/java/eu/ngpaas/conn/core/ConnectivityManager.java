...
public class ConnectivityManager implements PolicyService {
    ...
    protected void deactivate() {
        ...
        Response response = RESTtarget.path("policytype/deregister/connectivity").request(MediaType.APPLICATION_JSON)
                                      .delete();
    ...
    public SimpleResponse formalValidation(PolicyRule pr) {
        ...
        for (CopyOnWriteArrayList<PolicyCondition> clause : pr.getPolicyConditions()) {
            ...
            //if (clause.size() != 2) return new SimpleResponse("Formal error: A pair of source and destination
            // addresses must be provided.", false);
            for (PolicyCondition pc : clause) {
                if (!supported_conds.contains(pc.getPolicyVariable())) {
                    restResponse = new SimpleResponse("Formal error: Parameter " + pc.getPolicyVariable() + " invalid.",
                                                      false);
                    return restResponse;
                }
            }
        ...
        if (pr.getPolicyActions().size() > 1) {
            return new SimpleResponse("Formal error: Only one action is supported", false);
        }
        if (!supported_actions.contains(pr.getPolicyActions().get(0).getPolicyVariable()) || !pr.getPolicyActions()
                                                                                                .get(0).getPolicyValue()
                                                                                                .equalsIgnoreCase(
                                                                                                    "true")) {
    ...
    private HostToHostIntent getIntent(CopyOnWriteArrayList<PolicyCondition> pcs, PolicyAction pa,
        ...
        if (src.toString().compareTo(dst.toString()) < 0) {
            key = Key.of(src.toString() + dst.toString(), applicationId);
        } else {
            key = Key.of(dst.toString() + src.toString(), applicationId);
        }
